...
export function PhotoToTextTool() {
  ...
  const processFile = useCallback(
    async (file: File) => {
      if (!ACCEPTED_TYPES.includes(file.type)) {
        setError("Please upload a photo (JPG, PNG, GIF, BMP, or WebP).")
        setStatus("error")
        return
      }
      ...
      try {
        ...
        if (!result.text.trim()) {
          setError(
            "No text found in this photo. The image might be too blurry, low-resolution, or contain no readable text."
          )
          setStatus("error")
          return
        }
        ...
        setError(
          "Failed to extract text from this photo. Please try again with a clearer image."
        )
        setStatus("error")
      }
    },
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url
    a.download = fileName ? fileName.replace(/\.[^.]+$/, ".txt") : "photo-text.txt"
    a.click()
    ...
    <div className="w-full max-w-2xl mx-auto">
          ...
          >
            <div className="flex flex-col items-center justify-center py-14 px-6">
              ...
              <p className="text-sm text-muted-foreground">
                Photos in JPG, PNG, GIF, BMP, or WebP up to 20 MB
              </p>
            </div>
            <input
              ref={inputRef}
      ...
      {status === "processing" && (
        <div className="rounded-2xl border bg-card p-10 text-center">
          ...
          <p className="font-semibold text-lg mb-1">Extracting text from photo...</p>
