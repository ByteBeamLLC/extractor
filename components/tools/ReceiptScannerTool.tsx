...
export function ReceiptScannerTool() {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (!text.trim()) {
        setError(
          "No text could be extracted from this receipt. The image may be too blurry or low quality."
        )
        setStatus("error")
        return
      }
      ...
      setError(
        "Failed to process this file. It may be too blurry or in an unsupported format."
      )
      setStatus("error")
    }
  }, [])
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url
    a.download = `${fileName?.replace(/\.[^.]+$/, "") || "receipt"}-extracted.txt`
    a.click()
    ...
    <div className="w-full max-w-2xl mx-auto">
        ...
        >
          <div className="flex flex-col items-center justify-center py-14 px-6">
            ...
            <p className="text-sm text-muted-foreground">
              Photo or PDF of a receipt up to 50 MB
            </p>
          </div>
          <input
            ref={inputRef}
      ...
      {status === "processing" && (
        <div className="rounded-2xl border bg-card p-10 text-center">
          ...
          <p className="font-semibold text-lg mb-1">Scanning your receipt...</p>
          ...
          <div className="px-6 py-4 border-t">
            <div className="rounded-xl border-2 border-primary/20 bg-primary/5 p-5 text-center">
              ...
              <p className="font-semibold mb-1">
                Need structured receipt data instead of raw text?
              </p>
              <p className="text-sm text-muted-foreground mb-3">
                Parsli AI extracts merchant name, items, totals, tax, payment
                method, and more as structured JSON.
              </p>
      ...
      {status === "error" && (
        <div className="rounded-2xl border bg-card overflow-hidden">
          ...
          {fileTooLarge ? (
            <FileSizeBridgeBanner toolName="receipt_scanner" />
          ) : (
            <div className="px-6 py-4 border-t flex flex-col sm:flex-row gap-3">
