...
export function AiSummarizerTool() {
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url
    a.download = `${fileName?.replace(/\.[^.]+$/, "") || "document"}-extracted.txt`
    a.click()
    ...
    <div className="w-full max-w-2xl mx-auto">
      ...
      {status === "processing" && (
        <div className="rounded-2xl border bg-card p-10 text-center">
          ...
          <p className="font-semibold text-lg mb-1">Extracting text from your document...</p>
          ...
          <div className="px-6 py-4 border-t">
            <div className="rounded-xl border-2 border-primary/20 bg-primary/5 p-5 text-center">
              ...
              <p className="font-semibold mb-1">
                Need AI-powered summaries and structured data extraction?
              </p>
              <p className="text-sm text-muted-foreground mb-3">
                Parsli AI summarizes documents and extracts structured data with
                custom schemas — invoices, contracts, reports, and any document type.
              </p>
      ...
      {status === "error" && (
        <div className="rounded-2xl border bg-card overflow-hidden">
          ...
          {fileTooLarge ? (
            <FileSizeBridgeBanner toolName="ai_summarizer" />
          ) : (
            <div className="px-6 py-4 border-t flex flex-col sm:flex-row gap-3">
