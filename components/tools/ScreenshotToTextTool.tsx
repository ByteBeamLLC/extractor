...
export function ScreenshotToTextTool() {
  ...
  const processFile = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        if (!result.text.trim()) {
          setError(
            "No text found in this screenshot. The image might contain only graphics, icons, or non-textual content."
          )
          setStatus("error")
          return
        }
        ...
        setError(
          "Failed to extract text from this screenshot. Please try again with a clearer image."
        )
        setStatus("error")
      }
    },
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url
    a.download = fileName ? fileName.replace(/\.[^.]+$/, ".txt") : "screenshot-text.txt"
    a.click()
    ...
    <div className="w-full max-w-2xl mx-auto">
          ...
          >
            <div className="flex flex-col items-center justify-center py-14 px-6">
              ...
              <p className="text-sm text-muted-foreground">
                Screenshots in JPG, PNG, GIF, BMP, or WebP up to 20 MB
              </p>
            </div>
            <input
              ref={inputRef}
      ...
      {status === "processing" && (
        <div className="rounded-2xl border bg-card p-10 text-center">
          ...
          <p className="font-semibold text-lg mb-1">Extracting text from screenshot...</p>
      ...
      {status === "error" && (
        <div className="rounded-2xl border bg-card overflow-hidden">
          ...
          {fileTooLarge ? (
            <FileSizeBridgeBanner toolName="screenshot_to_text" />
          ) : (
            <div className="px-6 py-4 border-t flex flex-col sm:flex-row gap-3">
              ...
              <Button asChild className="h-10">
                <a
                  ...
                  data-cta-location="screenshot_tool_after_extract"
