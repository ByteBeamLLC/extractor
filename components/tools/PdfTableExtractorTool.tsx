...
export function PdfTableExtractorTool() {
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (isPDF) {
        setProgress("Extracting table data from PDF...")
  ...
  const handleDownload = useCallback(() => {
    ...
    a.href = url
    a.download = `${fileName?.replace(/\.[^.]+$/, "") || "table-data"}-extracted.txt`
    a.click()
    ...
    <div className="w-full max-w-2xl mx-auto">
      ...
      {status === "processing" && (
        <div className="rounded-2xl border bg-card p-10 text-center">
          ...
          <p className="font-semibold text-lg mb-1">Extracting table data...</p>
          ...
          <div className="px-6 py-4 border-t">
            <div className="rounded-xl border-2 border-primary/20 bg-primary/5 p-5 text-center">
              ...
              <p className="font-semibold mb-1">
                Want perfectly structured table data instead of raw text?
              </p>
              <p className="text-sm text-muted-foreground mb-3">
                Parsli AI extracts tables into clean rows and columns as
                structured JSON or Excel — even from complex layouts and scanned documents.
              </p>
      ...
      {status === "error" && (
        <div className="rounded-2xl border bg-card overflow-hidden">
          ...
          {fileTooLarge ? (
            <FileSizeBridgeBanner toolName="pdf_table_extractor" />
          ) : (
            <div className="px-6 py-4 border-t flex flex-col sm:flex-row gap-3">
